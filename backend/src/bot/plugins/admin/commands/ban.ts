...
export const execute = async function({ message, args }: CommandArgs) {
	...
	if (!mention) {
		if ((args[0] == "594526434526101527" && message.author.id !== "364056796932997121") ||(args[0] == "364056796932997121" && message.author.id !== "594526434526101527")) return message.react("🖕");
		...
		if ((mention.id == "594526434526101527" && message.author.id !== "364056796932997121") || (mention.id == "364056796932997121" && message.author.id !== "594526434526101527")) return message.react("🖕");
		await mention
