...
export const execute = async function({ client, message, args }: CommandArgs) {
	if (!args[0]) return message.channel.send({
		embeds: [ErrorEmbed(message, "Prawidłowe użycie: `.editdonate <ID>`")]
	});
	...
	if (!args[1] || !["amount", "userID", "type", "approved", "dID"].includes(args[1])) return message.channel.send({ content: `\`\`\`${ JSON.stringify(donate) }\`\`\`` });
	else if (!args[2]) return message.channel.send({
	...
	message.channel.send({ content: `**Zapisano!**\n\`\`\`${ JSON.stringify(donate) }\`\`\`` });
};
