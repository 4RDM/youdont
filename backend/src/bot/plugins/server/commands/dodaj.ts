...
export const execute = async function({ message, args }: CommandArgs) {
	...
	if (args.length < 3 || !mention)
		return message.channel.send({
			embeds: [ErrorEmbed(message, "Prawidłowe użycie: `.dodaj <hex> <ranga> <@ping>`")],
		});
	const msg = await message.channel.send("Wysyłanie");
	addFile(`add_principal identifier.steam:${args[0]} group.${args[1]} # ${mention.tag} (${mention.id}) https://steamcommunity.com/profiles/${hexToDec(args[0])} ${new Date().toLocaleDateString()}`, "F:\\fivem-server\\base\\permisje.cfg")
		.then(() => {
			msg.edit("Wysłano");
			// TODO
