...
const check = (s: { result: string }) => s.result == "malware" || s.result == "phishing" || s.result == "malicious";
...
export const checkMessage = async (message: string): Promise<boolean> => {
    try {
        ...
        const results = await fetch(baseURL, { method: "POST", headers: { "x-apikey": virustotal }, body: form });
