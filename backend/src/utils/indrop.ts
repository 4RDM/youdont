        ...
        try {
            ...
            if (discord) {
                if (!payment.product_id.startsWith("unban") && !payment.product_id.startsWith("ranga")) {
                    this.sendWebhook(this.newDiscord, [{ name: "Użytkownik", value: `<@${discord.replace("discord:", "")}> (${discord})`, inline: true }, { name: "Zakupiony przedmiot", value: `\`${payment.product_id}\``, inline: true }, { name: "Hex", value: hex, inline: true }], "#4fdf62", "Wpłata");
                }
